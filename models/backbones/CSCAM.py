...
class CSCAModule(nn.Module):
    ...
    def __init__(self, d_model=512, kernel_size=3, H=7, W=7,h=1):
        super().__init__()
        self.position_attention_module = PositionAttentionModule(d_model=d_model, kernel_size=kernel_size, H=H, W=W,head=h)
        self.channel_attention_module = ChannelAttentionModule(d_model=d_model, kernel_size=kernel_size, H=H, W=W,head=h)
        self.gamma = nn.Parameter(torch.zeros(1), requires_grad=True)
...
class CSCAM(nn.Module):
    def __init__(self,
        ...
        # 修改：动态计算 H 和 W
        h_side = int(math.sqrt(sequence_length))
        self.sequence_length = sequence_length  # 保存以便检查
        self.CSCAModule = CSCAModule(d_model=embedding_dim, kernel_size=3, H=h_side, W=h_side, h=1)
